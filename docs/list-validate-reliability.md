...
一条可选项建议称为 **Profile（运行配置）**，而 `provider` 专指 DeepSeek、OpenAI 这样的服务提供商。当前每条 `[[providers]]` 还包含 agent 工具、模型、环境变量和启动参数，叫整个对象 `Provider` 会与其中的 `provider` 字段重名且语义不准。命令参数直接改为 `-p, --profile <PROFILE_ID>`；不保留 `--provider` 别名；旧写法给出改用 `--profile` 的错误提示。`[[providers]]`、`provider` 字段与既有 ID 不迁移。内部 `Provider` 类型改为 `Profile`，不改变序列化格式。
...
接口、持久化数据格式、启动命令构造规则均无变化。**DDL：无变化。**
...
- 整条可启动配置称为 Profile；`provider` 只表示服务商，现有配置键不更名，`-p` 保留，`--provider` 直接更名为 `--profile`。
